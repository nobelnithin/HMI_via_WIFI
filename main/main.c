...
{
    // Seed the random number generator (only once, typically in app_main)
    int random_num = (rand() % 199) - 99;  // Generates a number between -99 and 99

    char response[32];
    snprintf(response, sizeof(response), "%d", random_num);
    httpd_resp_send(req, response, HTTPD_RESP_USE_STRLEN);

    ESP_LOGI(TAG, "Sent random number: %d", random_num);
    return ESP_OK;
}
